...
void Connector::connect()//具体的连接函数
{
...
void Connector::connecting(int sockfd)//连接成功的处理函数
{
  setState(kConnecting);
  assert(!channel_);
  // Channel与sockfd关联
  channel_.reset(new Channel(loop_, sockfd));
  // 设置可写回调函数，这时候如果socket没有错误，sockfd就处于可写状态，这只是为了测试socket连接没有问题
...
int Connector::removeAndResetChannel()
{
  ...
  loop_->queueInLoop(boost::bind(&Connector::resetChannel, this)); // FIXME: unsafe//将channel_置空
  return sockfd;
}
...
void Connector::handleWrite()
{
  ...
  if (state_ == kConnecting)
  {
    ...
    else	// 连接成功，就执行newConnectionCallback_回调函数
    {
      setState(kConnected);
      if (connect_)
      {
...
void Connector::handleError()//处理错误的回调函数
{
...
// back-off策略就是第一次用0.5秒连接，然后使用1秒连接，重新连接的时间逐渐延长，类似于一个递归函数，一直在递归，
// ？？？个人感觉不会停止，到最后就是过30s，尝试连接一次，没有成功，就继续等待30s，应该就是一直在重连，除非人为打断。
//
