...
FileUtil::SmallFile::SmallFile(StringPiece filename)
  : fd_(::open(filename.data(), O_RDONLY | O_CLOEXEC)),//打开文件，并且是只读模式，并且开启新的子进程时，文件描述符就会自动关闭
    err_(0)
{
  buf_[0] = '\0';
  if (fd_ < 0)
...
FileUtil::SmallFile::~SmallFile()//关闭文件
{
  if (fd_ >= 0)
  {
                                      ...
                                      int64_t* createTime)
{
  ...
  if (fd_ >= 0)
  {
    ...
    if (fileSize)
    {
      ...
      {
        if (S_ISREG(statbuf.st_mode))//如果是常规文件
        {
